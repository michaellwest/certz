...
## Trust Store Tests in Containers

Trust tests that target the Root store use `LocalMachine\Root` instead of `CurrentUser\Root`. This avoids the Windows Security UI dialog that would hang in non-interactive container environments. The Docker Server Core container runs as `ContainerAdministrator`, so all `LocalMachine` store operations work without issues.

All test suites, including trust tests, are safe to run in Docker containers:

```powershell
# All test suites work in Docker (no UI dialog hangs)
...
See [testing.md — Known Limitations](testing.md#known-limitations) for details on the auto-detection behavior.
